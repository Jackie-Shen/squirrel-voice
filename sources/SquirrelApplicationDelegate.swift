...
final class SquirrelApplicationDelegate: NSObject, NSApplicationDelegate, SPUStandardUserDriverDelegate, UNUserNotificationCenterDelegate {
  ...
  /// VoiceNative：当前激活的输入控制器（activateServer 时注册）。语音事件投递不能依赖
  /// panel.inputController——它只在 rime 显示过候选窗后才有值，而热键/定稿需要随时可投递。
  var activeInputController: SquirrelInputController?
  /// VoiceNative：原生语音输入协调器（enabled=false 时为 nil）。
  var voiceNative: VoiceNativeCoordinator?
  /// VoiceNative：设置窗口（懒创建，复用同一实例）。
  var settingsWindow: VoiceNativeSettingsWindow?
  ...
  func applicationWillFinishLaunching(_ notification: Notification) {
    ...
    refreshStatusItem()
    addObservers()
    // VoiceNative：启动原生语音输入（读 UserDefaults voice_native.*；禁用时为 nil）。
    voiceNative = VoiceNativeCoordinator.start(delegate: self)
  }
  ...
  /// VoiceNative：打开设置界面（热键展示 + LLM 清洗 + 说话人过滤）。
  func openSettings() {
    if settingsWindow == nil {
      settingsWindow = VoiceNativeSettingsWindow()
    }
    // accessory 应用反复 activate 不可靠（关闭后第二次打不开）：显示前临时切 .regular，
    // 窗口关闭时由 windowWillClose 恢复 .accessory。
    if NSApp.activationPolicy() == .accessory {
      NSApp.setActivationPolicy(.regular)
    }
    NSApp.activate(ignoringOtherApps: true)
    settingsWindow?.makeKeyAndOrderFront(nil)
    // IME 是 windowless 后台进程，makeKeyAndOrderFront 第二次可能不生效；
    // orderFrontRegardless 强制提到前台，更可靠。
