...
final class SquirrelPanel: NSPanel {
  ...
  override func sendEvent(_ event: NSEvent) {
    ...
    case .leftMouseUp:
      // VoiceNative：语音会话期间点击候选行 = 提交（不走 rime selectCandidate）。
      if inputController?.voiceActive == true {
        _ = inputController?.voiceCommit()
        break
      }
  ...
  func hide() {
    // 调试：语音会话期间面板被藏 = bug，记录调用栈定位来源。
    if inputController?.voiceActive == true {
      let stack = Thread.callStackSymbols.prefix(8).joined(separator: " | ")
      SquirrelVoiceLog.write("panel.hide()（语音会话中！）栈：\(stack)")
    }
