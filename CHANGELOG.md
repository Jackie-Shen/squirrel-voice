# SquirrelVoice（Squirrel fork）变更日志

> 本 fork（macOS 原生语音输入）自 **1.2.x** 起独立发版，分发包为 `releases/SquirrelVoice-<版本>.dmg`
> （构建/安装见 [BUILD_AND_INSTALL.md](BUILD_AND_INSTALL.md)，设计见 [DESIGN.md](DESIGN.md)），
> releases 托管于 [Jackie-Shen/squirrel-voice](https://github.com/Jackie-Shen/squirrel-voice)。
> 以下 **1.1.x 及更早为上游 [rime/squirrel](https://github.com/rime/squirrel)** 的历史，保留作参考。
> 后续 fork 条目用 `package/bump_version`（git-cliff）在获得提交记录后生成。
