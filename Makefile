...
release: $(DEPS_CHECK)
	...
	bash package/add_rime_ice "$(DERIVED_DATA_PATH)" Release
...
debug: $(DEPS_CHECK)
	...
	bash package/add_rime_ice "$(DERIVED_DATA_PATH)" Debug
...
# DMG 安装包：卷内含 Squirrel.app + 安装.command + 卸载.command（依赖已构建的 Release）
dmg:
	bash package/make_dmg "$(DERIVED_DATA_PATH)"
