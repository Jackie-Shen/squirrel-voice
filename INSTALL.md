...
> **本倉庫（SquirrelVoice）構建請優先看 [BUILD_AND_INSTALL.md](BUILD_AND_INSTALL.md)** —
> 那是針對此 fork（macOS 26 + arm64 + VoiceNative 語音模塊、.pkg / DMG 雙產物）的實際流程。
> 以下為上游通用說明，僅作參考（部分步驟如 universal / Xcode 14 / rime-ice 與本 fork 不同）。
...
``` sh
git clone --recursive https://github.com/Jackie-Shen/squirrel-voice.git
